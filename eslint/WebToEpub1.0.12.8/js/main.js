...
*/
var main = (function() { // eslint-disable-line no-unused-vars
    "use strict";
    ...
    async function fetchContentAndPackEpub() {
        ...
        /**
         * Re-enable UI here because save-prompt promise won't return if the
         * user closes/cancels the window.
         */
        setUIWorkingState(false);
    ...
    /**
     * Function for toggling the UI state to indicate whether work is in
     * progress. Such as disabling buttons, swapping elements, and asking user
     * before closing the page while a download is in progress.
     * 
     * @param { boolean } isWorking Whether work is in progress.
     * @returns { void }
     */
    function setUIWorkingState(isWorking) {
        if (typeof isWorking != "boolean") {
            throw new Error(`Invalid argument; must be a boolean. Found: ${typeof isWorking} - ${isWorking}`);
        }

        // Toggle prevent page closing.
        window.workInProgress = isWorking;
         ...
         */
        let elements = document.querySelectorAll("input, select, button:not(#LibPauseToLibrary), textarea");
        
        elements.forEach(element => element.disabled = isWorking);
    }
